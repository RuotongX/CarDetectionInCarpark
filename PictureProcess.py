...
out = cv2.VideoWriter('out2.avi', fourcc, fps, (outw, outh))
...
def read_xml_test(filename):
    ...
    center = root.getElementsByTagName('center')
    size = root.getElementsByTagName('size')
    temp = []
    for i in range(len(size)):
        cx = int(center[i].getAttribute("x"))
        cy = int(center[i].getAttribute("y"))
        sx = int(size[i].getAttribute("w"))
        sy = int(size[i].getAttribute("h"))
        point1x = cx-sx/2
        point1y = cy-sy/2
        point2x = cx+sx/2
        point2y = cy+sy/2
        box = [int(point1x),int(point1y),int(point2x),int(point2y),0]
        temp.append(box)
    return temp
...
while True:
    ...
    for i in range(len(detectionlist)):
        if (detectionlist[i][1] == 3 or detectionlist[i][1] == 6 or detectionlist[i][1] == 8) and detectionlist[i][2] > 0.2:
    ...
    # [195,76,307,229,0],[365,63,490,244,0],[500,19,640,220,0]
    filename = 'PKlot/2013-04-12_06_55_01.xml'
    parkingbox = read_xml_test(filename)
    ...
    for i in range(len(parkingbox)):
        if parkingbox[i][4] == 1:
            ...
            # print((parkingbox[i][0],parkingbox[i][1]),(parkingbox[i][2],parkingbox[i][3]))
